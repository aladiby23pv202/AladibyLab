...
//            bank.setNumberOfAtms(bank.getNumberOfAtms() + 1);
    ...
    @Override
    public Bank getBankById(ArrayList<Bank> banks, int bankId) {
        for (Bank bank : banks) {
    ...
    @Override
    public boolean transferClient(User user, int newBankId, ArrayList<Bank> allBanks, BankService bankService) {
        Bank newBank = bankService.getBankById(allBanks, newBankId);
        if (newBank != null) {
            for (Bank currentBank : user.getBanks()) {
                bankService.deleteClient(currentBank, user);
            }
            user.setBanks(new ArrayList<>());
            user.addBank(newBank);
            bankService.addClient(newBank, user);
            return true;
        }
        return false;
    }
